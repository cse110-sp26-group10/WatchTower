...
const MODE = window.location.pathname.endsWith("feedback.html")
  ? "feedback"
  : "error";
const CONFIG = {
  error: {
...
function loadResolvedIds() {
  try {
    return new Set(JSON.parse(localStorage.getItem(RESOLVED_IDS_KEY) || "[]"));
  } catch {
    return new Set();
  }
}
function loadResolvedGroups() {
  try {
    return new Map(
      Object.entries(
        JSON.parse(localStorage.getItem(RESOLVED_GROUPS_KEY) || "{}"),
      ),
    );
  } catch {
    return new Map();
...
function syncPanelState() {
  document
    .querySelectorAll("#errors-list details[data-signal-id]")
    .forEach((panel) => {
      if (panel.open) {
...
function getSignals() {
  return window.WatchTowerData.getEvents({ deploymentId: activeDeploymentId })
...
function renderHeader(signals) {
  ...
  const lowRatingCount = signals.filter(
    (e) =>
      Number(e.metadata.rating || 0) > 0 && Number(e.metadata.rating || 0) <= 2,
  ).length;
  const midRatingCount = signals.filter(
    (e) => Number(e.metadata.rating || 0) === 3,
  ).length;
...
function renderSignalPanel(signal) {
  ...
  const detailRows = [
    ...
    kvRow(
      "observed",
      `${relativeTime(signal.timestamp)} (${new Date(signal.timestamp).toLocaleString()})`,
    ),
    kvRow(
      "received",
      `${relativeTime(signal.created_at)} (${new Date(signal.created_at).toLocaleString()})`,
    ),
  ];
  const deploymentRows = dep
    ? [
        ...
        fullDep && fullDep.deployed_at
          ? kvRow(
              "deployed",
              `${relativeTime(fullDep.deployed_at)} (${new Date(fullDep.deployed_at).toLocaleString()})`,
            )
          : "",
      ].join("")
...
function groupErrors(signals) {
  ...
  for (const s of signals) {
    ...
    g.signals.push(s);
    if (
      (SEVERITY_RANK_MAP[s.metadata.severity] ?? 99) <
      (SEVERITY_RANK_MAP[g.severity] ?? 99)
    ) {
  ...
  return Array.from(groups.values()).sort((a, b) => {
    const sevDiff =
      (SEVERITY_RANK_MAP[a.severity] ?? 99) -
      (SEVERITY_RANK_MAP[b.severity] ?? 99);
...
function renderGroupedErrorPanel(group) {
  ...
  const countBadge =
    count > 1 ? `<span class="error-count">\xd7${count}</span>` : "";
  ...
  const detailRows = [
    ...
    kvRow(
      "last seen",
      `${relativeTime(s.timestamp)} (${new Date(s.timestamp).toLocaleString()})`,
    ),
    count > 1
      ? kvRow(
          "first seen",
          `${relativeTime(firstTs)} (${new Date(firstTs).toLocaleString()})`,
        )
      : "",
...
function attachResolveHandlers() {
  document.getElementById("errors-list").addEventListener(
    "click",
    (e) => {
...
function renderSignals({ force = false } = {}) {
  ...
  const signals =
    MODE === "error" ? allSignals.filter((e) => !isResolved(e)) : allSignals;

  const renderKey =
    signals.map((e) => e.id + e.timestamp).join(",") + activeDeploymentId;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("errors-title").nextElementSibling.textContent =
    pageConfig.hint;
