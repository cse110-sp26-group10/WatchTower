...
function renderCart() {
  ...
  const rows = items.map((item) => {
    ...
    return (
      ...
      escapeHtml(lineTotal.toFixed(2)) +
      "</div>" +
  ...
  totalsEl.innerHTML =
    '<div class="cart-total-line">Subtotal: $' +
    subtotal.toFixed(2) +
    "</div>" +
...
function triggerUnhandledRejection() {
  Promise.reject(
    new Error("WatchTower test — unhandled promise rejection from cart page"),
  );
}
