...
|----|------|-------------|--------|-------|
| T01 | Complete backend-frontend integration (connect new UI to endpoints) | Han Yang-Lin, Kevin Wang, Jensen Guo | ✅ Done | Home, errors, feedback, activity, and projects pages all fetch live data |
| T02 | Write ADR for notification system (NodeMailer / ntfy) | Han Yang-Lin, Kevin Wang | ✅ Done | ADR 007 written and merged |
| T03 | Update Supabase ADR to include auth reasoning | Han Yang-Lin, Kevin Wang | ✅ Done | ADR 006 already included auth rationale; confirmed complete |
...
| T05 | Merge browser-type detection PR and validate in dashboard | Evan Marriott, Nicole Sutedja | ✅ Done | Browser field visible in issue detail context panel |
| T06 | Implement screen/display size tracking for error events | Evan Marriott, Nicole Sutedja | 🔲 Not Started | Carried from Sprint 3; deprioritized for code freeze |
| T07 | Finalize PR structure and close completed GitHub Issues | Evan Marriott, Nicole Sutedja | 🔄 In Progress | Issues being triaged |
| T08 | Write "definition of done" into process docs | Evan Marriott, Nicole Sutedja | ✅ Done | DoD checklist embedded in docs/pr-template.md and required on all PRs |
| T09 | Finalize CI pipeline additions (dependency check, changelog, formatting) & open PR from testing branch | Benedict Luis, Aron Wu | ✅ Done | All CI additions merged into main |
| T10 | Check and update all Scrum Master docs, repo tasks, metrics, and issues | Benedict Luis, Aron Wu | 🔄 In Progress | In progress — sprint docs and metrics being finalized |
| T11 | Rename `prototype` folder and update lint/test paths | Evan Marriott, Nicole Sutedja | ✅ Done | `src/app/` is now the primary directory; prototype archived |
...
| Han Yang-Lin | fixed survey message display, added project filter to top bar and removed it from the uptime card, fixed uptime bar margin, added test user and deployed project to seed.sql, implemented project sharing with permission levels (Owner, Co-Owner, Viewer), fixed API key in tracker.js | do a final backend review before code freeze | None |
...
| Prakhar Shah | audited sprint docs, ADRs, and GENAI log — filled in empty stand-up entries, fixed ADR language and incomplete sections, fixed GENAI typo | finish any remaining documentation gaps, update README | None |
...
|----------|-----------|----------|
| Add permission levels (Owner, Co-Owner, Viewer) to project sharing | Sharing a project with someone should let the owner control what that person can do; Viewer access covers read-only use and Co-Owner allows management tasks | |
...
* Finish any UI fixes necessary — be very detail oriented
* Overall polish the app and cards, make it look more professional and ready for demo. Design decisions should be grounded in developer user research/needs.
...
* Everything should be merged into main
* Check all Scrum Master docs/repo are up to date — tasks need to be marked done if yes, metrics updated, issues up to date — put comments on all closed/open issues
* Shift to Demo Readiness team once done with these tasks
