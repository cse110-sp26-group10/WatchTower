...
test.describe('Dashboard — authenticated', () => {
  ...
  test('error list and activity panels render with items', async ({ page }) => {
    ...
    await expect(errorList.locator('.error-click-target-btn').first()).toBeVisible();
  ...
  test('clicking an error row opens the detail modal', async ({ page }) => {
    await page.goto('/#/');
    const errorBtn = page.locator('#home-errors .error-click-target-btn').first();
  ...
  test('error detail modal shows severity badge and error info', async ({ page }) => {
    await page.goto('/#/');
