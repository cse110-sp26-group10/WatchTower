...
We should avoid writing too many tests for the visual design right now because the UI may still change-get the portotype/dashboard layout done first.
...
2. See the main overview with recent errors, performance warnings, upset user signals, and deployment/build signals.
3. Use a basic filter, such as severity or event type.
4. Click or select an issue from the dashboard.
5. Open the issue detail view.
6. Check that the issue detail shows the severity, timestamp, related users or feedback notes, and related deployment/build signal.
7. Go back to the dashboard.
8. If the feedback widget is included, submit a simple rating or feedback signal and show that it appears as an upset user signal.
...
For Sprint 1 there's still not much testing needed, but a manual testing is enough as long as we show clear evidence. Before submitting, we should check that:
