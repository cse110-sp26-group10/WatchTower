...
describe("Event — valid construction", () => {
  it("constructs a valid page_load event", () => {
    const e = new Event(
      JSON.stringify(validBase("page_load", { load_time: 150 })),
    );
  ...
  it("constructs a valid error event", () => {
    const e = new Event(
      JSON.stringify(
        validBase("error", { severity: "critical", message: "boom" }),
      ),
    );
    expect(e.valid).toBe(true);
  ...
  it("constructs a valid survey event", () => {
    const e = new Event(
      JSON.stringify(validBase("survey", { rating: 4, message: "nice" })),
    );
    expect(e.valid).toBe(true);
  });
...
describe("Event — event_type validation", () => {
  it("rejects an unknown event_type", () => {
    expect(
      new Event(JSON.stringify({ ...validBase(), event_type: "unknown" }))
        .valid,
    ).toBe(false);
  });
  ...
  it("rejects a numeric event_type", () => {
    expect(
      new Event(JSON.stringify({ ...validBase(), event_type: 42 })).valid,
    ).toBe(false);
  });
});
...
describe("Event — timestamp validation", () => {
  it("rejects a timestamp more than 300 seconds in the past", () => {
    expect(
      new Event(JSON.stringify({ ...validBase(), timestamp: secsAgo(400) }))
        .valid,
    ).toBe(false);
  });
  ...
  it("rejects a future timestamp", () => {
    ...
    expect(
      new Event(JSON.stringify({ ...validBase(), timestamp: future })).valid,
    ).toBe(false);
  });

  it("rejects a non-ISO date string", () => {
    expect(
      new Event(JSON.stringify({ ...validBase(), timestamp: "2026-05-19" }))
        .valid,
    ).toBe(false);
  });
});
...
describe("Event — deployment validation", () => {
  ...
  it("rejects a future deployed_at", () => {
    ...
    const data = {
      ...validBase(),
      deployment: { ...validBase().deployment, deployed_at: future },
    };
