...
import {attemptSuccess, UptimeCheckAttempt, UptimeCheck} from '../../src/prototype/server/assets/UptimeCheck.js';
...
describe('UptimeCheck',() => {
    ...
    it('Marks check as down when last attempt has null status (fetch error)', () => {
        const attempts = [mkAttempt(0, 200), mkAttempt(1000, null, new Error('network error'))];
        const check = new UptimeCheck('https://example.com', attempts);
        expect(check.is_up).toBe(false);
