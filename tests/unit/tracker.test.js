...
describe("parseBrowser — userAgentData path", () => {
  it("uses userAgentData when brands are present", () => {
    ...
    expect(parseBrowser(CHROME_UA, uaData)).toEqual({
      name: "Google Chrome",
      version: "124",
    });
  });
  ...
  it("falls through to regex when uaData is undefined", () => {
    expect(parseBrowser(FIREFOX_UA, undefined)).toEqual({
      name: "Firefox",
      version: "125",
    });
  });
...
describe("parseBrowser — regex path", () => {
  it("detects Chrome", () => {
    expect(parseBrowser(CHROME_UA, undefined)).toEqual({
      name: "Chrome",
      version: "124",
    });
  });
  ...
  it("detects Edge (not misidentified as Chrome)", () => {
    expect(parseBrowser(EDGE_UA, undefined)).toEqual({
      name: "Edge",
      version: "124",
    });
  });
  ...
  it("detects Safari (not misidentified as Chrome)", () => {
    expect(parseBrowser(SAFARI_UA, undefined)).toEqual({
      name: "Safari",
      version: "17",
    });
  });
  ...
  it("detects Opera (not misidentified as Chrome)", () => {
    expect(parseBrowser(OPERA_UA, undefined)).toEqual({
      name: "Opera",
      version: "110",
    });
  });
...
describe("parseBrowser — fallback path", () => {
  ...
  it('returns "Unknown" when the UA string is empty', () => {
    expect(parseBrowser("", undefined)).toEqual({
      name: "Unknown",
      version: "",
    });
  });
